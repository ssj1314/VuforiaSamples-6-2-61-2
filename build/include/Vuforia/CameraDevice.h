...
public:
    ...
    enum FOCUS_MODE 
    {
        FOCUS_MODE_NORMAL,           ///< Default focus mode 默认
        FOCUS_MODE_TRIGGERAUTO,      ///< Triggers a single autofocus operation 触发一个自动对焦
        FOCUS_MODE_CONTINUOUSAUTO,   ///< Continuous autofocus mode 连续自动对焦模式
        FOCUS_MODE_INFINITY,         ///< Focus set to infinity 焦点设置为无穷大
        FOCUS_MODE_MACRO             ///< Macro mode for close-up focus 微距对焦模式
    };

    enum CAMERA_DIRECTION
    { 
        CAMERA_DIRECTION_DEFAULT,   /// Default camera direction (device-specific, usually maps to back camera) 相机默认方向（具体设备，通常映射到相机背面）
        CAMERA_DIRECTION_BACK,      /// The camera is facing in the opposite direction as the screen 相机正朝相反的方向作为屏幕
        CAMERA_DIRECTION_FRONT,     /// The camera is facing in the same direction as the screen 相机与屏幕的方向相同
    };
