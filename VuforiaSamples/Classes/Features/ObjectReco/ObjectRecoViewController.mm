...
{
    [self performSelector:@selector(cameraPerformAutoFocus) withObject:nil afterDelay:.4];//cameraPerformAutoFocus相机执行自动对焦
}

- (void)cameraPerformAutoFocus//相机执行自动对焦
{
    Vuforia::CameraDevice::getInstance().setFocusMode(Vuforia::CameraDevice::FOCUS_MODE_TRIGGERAUTO);//CameraDevice相机设备 getInstance得到实例  setFocusMode设置对焦模式   FOCUS_MODE_TRIGGERAUTO触发一个自动对焦
    
    // After triggering an autofocus event,
    // we must restore the previous focus mode
    //触发后自动聚焦的事件，我们必须恢复以前的对焦模式
    if (continuousAutofocusEnabled)// continuousAutofocusEnabled连续自动对焦功能
    {
        [self performSelector:@selector(restoreContinuousAutoFocus) withObject:nil afterDelay:2.0];// restoreContinuousAutoFocus恢复连续自动对焦
        
    }
}
//restoreContinuousAutoFocus恢复连续自动对焦
- (void)restoreContinuousAutoFocus
{
    Vuforia::CameraDevice::getInstance().setFocusMode(Vuforia::CameraDevice::FOCUS_MODE_CONTINUOUSAUTO);//setFocusMode设置对焦模式  FOCUS_MODE_CONTINUOUSAUTO连续自动对焦模式
}
//双击手势动作
- (void)doubleTapGestureAction:(UITapGestureRecognizer*)theGesture
{
    if (!self.showingMenu) {//显示菜单
...
- (BOOL)activateDataSet:(Vuforia::DataSet *)theDataSet
{
    ...
    if (objectTracker == NULL) {
        NSLog(@"Failed to load tracking data set because the ObjectTracker has not been initialized.未能加载跟踪数据集，因为objecttracker尚未初始化450");
    }
    else
    {
        // Activate the data set:启动数据集
        if (!objectTracker->activateDataSet(theDataSet))
        {
            NSLog(@"Failed to activate data set.激活数据集失败457");
        }
        else
        {
            NSLog(@"Successfully activated data set.成功激活数据集461");
            success = YES;
        }
    }
    ...
    //我们将关闭目标跟踪模式设置为当前状态
    if (success) {
        [self setExtendedTrackingForDataSet:theDataSet start:extendedTrackingEnabled];//setExtendedTrackingForDataSet设置扩展跟踪数据   extendedTrackingEnabled启用扩展跟踪
    }
...
- (BOOL)deactivateDataSet:(Vuforia::DataSet *)theDataSet
{
    ...
    if (objectTracker == NULL)
    {
        NSLog(@"Failed to unload tracking data set because the ObjectTracker has not been initialized.未能卸载跟踪数据集，因为objecttracker尚未初始化489");
    }
    else
    {
        ...
        {
            NSLog(@"Failed to deactivate data set.未能关闭数据集496");
        }
        else
        {
            success = YES;
...
//setExtendedTrackingForDataSet设置扩展跟踪数据集
- (BOOL) setExtendedTrackingForDataSet:(Vuforia::DataSet *)theDataSet start:(BOOL) start {
    ...
    for (int tIdx = 0; tIdx < theDataSet->getNumTrackables(); tIdx++) {
        ...
        if (start) {
            ...
            {
                NSLog(@"Failed to start extended tracking on:无法开始扩展跟踪 %s515", trackable->getName());
                result = false;
            }
        } else {
            if (!trackable->stopExtendedTracking())//停止扩展追踪
            {
                NSLog(@"Failed to stop extended tracking on:停止扩展跟踪失败521 %s", trackable->getName());
                result = false;
            }
        }
    }
...
- (BOOL) menuProcess:(NSString *)itemName value:(BOOL)value
{
    ...
    if ([@"Flash" isEqualToString:itemName]) {
        bool result = Vuforia::CameraDevice::getInstance().setFlashTorchMode(value); //setFlashTorchMode 设置闪光灯模式
        flashEnabled = value && result;
        return result;
    }
    else if ([@"Extended Tracking" isEqualToString:itemName]) {
        bool result = [self setExtendedTrackingForDataSet:currentDataSet start:value];//设置扩展跟踪数据集
        if (result) {
            [eaglView setOffTargetTrackingMode:value];//建立了目标跟踪模式
        }
        extendedTrackingEnabled = value && result;//跟踪功能的扩展
        return result;
    }
    else if ([@"Autofocus" isEqualToString:itemName]) {//自动对焦
        ...
        continuousAutofocusEnabled = value && result;//连续自动对焦功能
        return result;
    }
    else if ([@"Front" isEqualToString:itemName]) {//前
        if ([vapp stopCamera:&error]) {
            bool result = [vapp startAR:Vuforia::CameraDevice::CAMERA_DIRECTION_FRONT error:&error];//相机与屏幕的方向相同
    ...
    else if ([@"Rear" isEqualToString:itemName]) {//后
        if ([vapp stopCamera:&error]) {
            bool result = [vapp startAR:Vuforia::CameraDevice::CAMERA_DIRECTION_BACK error:&error];//相机正朝相反的方向作为屏幕
            frontCameraEnabled = !result;
            return result;
        } else {
            return false;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue isKindOfClass:[PresentMenuSegue class]]) {
        ...
        if ([dest isKindOfClass:[SampleAppMenuViewController class]]) {
            ...
            SampleAppMenuViewController *menuVC = (SampleAppMenuViewController *)dest;//App 菜单视图控制器
            menuVC.menuDelegate = self;
            menuVC.sampleAppFeatureName = @"Object Reco";//FeatureName功能名称   Object Reco物体的识别
            menuVC.dismissItemName = @"Vuforia Samples";//返回项目名称
            menuVC.backSegueId = @"BackToObjectReco";//回到物体识别
            
            // initialize menu item values (ON / OFF) 初始化菜单项值（开/关）
            [menuVC setValue:extendedTrackingEnabled forMenuItem:@"Extended Tracking"];//扩展跟踪功能 菜单项为  延长跟踪
            
            [menuVC setValue:continuousAutofocusEnabled forMenuItem:@"Autofocus"];//连续自动对焦功能   菜单项为  自动对焦
            
            [menuVC setValue:flashEnabled forMenuItem:@"Flash"];//闪光灯启用
            
            [menuVC setValue:frontCameraEnabled forMenuItem:@"Front"];//前置摄像头功能
            
            [menuVC setValue:!frontCameraEnabled forMenuItem:@"Rear"];//后置摄像头功能！！！！！！！！！！！
        }
    }
}
